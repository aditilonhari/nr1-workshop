...
export default class MyNerdlet extends React.Component {
  ...
  confirmFacet(e) {
    ...
    this.popToast(
      "normal",
      "Facet Updated",
      `The FACET ${this.state.value} has been added to your query.`
    );
  ...
  rejectFacet(e) {
    ...
    this.popToast(
      "critical",
      "Facet Rejected",
      `The FACET ${this.state.value} has been rejected.`
    );
        ...
        <ChartGroup>
          <Grid>
            <GridItem columnSpan={8}>
                  ...
                  <Stack>
                    <StackItem>
                      <h1 className="Modal-headline">
                        Are you sure you want to apply this facet?
                      </h1>
                      <p className="facet-value">
                        Facet by: <strong>{this.state.value}</strong>
                      </p>
                      <Stack>
                        <StackItem>
                          <Button onClick={this.rejectFacet}>No</Button>
                        </StackItem>
                        <StackItem>
                          <Button onClick={this.confirmFacet}>Yes</Button>
                        </StackItem>
                      </Stack>
                    </StackItem>
                  </Stack>
              ...
              <Stack fullWidth gapType={Stack.GAP_TYPE.LOOSE}>
                <StackItem grow={true}>
                  <AreaChart
                    query={throughput + since + this.state.facet}
                    accountId={this.accountId}
                ...
                <StackItem grow={true}>
                  <BarChart
                    className="two-col-chart"
                    query={errors + since + this.state.facet}
                    accountId={this.accountId}
                  />
                </StackItem>
              </Stack>
              ...
              >
                <StackItem grow={true}>
                  <PieChart
                    className="chart"
                    query={
                      transaction_apdex_by_appname + since + this.state.facet
                    }
                    accountId={this.accountId}
                  />
                </StackItem>
